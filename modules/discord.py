# Sample Program for Discord Server (Modified for Python 3.4+)
import discord
import asyncio
...
def on_message(message):
    if message.content.startswith('!test'):
        ...
        for log in client.logs_from(message.channel, limit=100):
          #yield from client.send_message(message.channel, 'Log.Author: ' + log.author)
          yield from client.send_message(message.channel, 'Message.Author: %s' % message.author)
